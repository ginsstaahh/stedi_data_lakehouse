...
object GlueApp {
  def main(sysArgs: Array[String]) {
    ...
    val customerlandingzone_node1700529677557 = glueContext.getCatalogSource(database="sparkify", tableName="customer_trusted", transformationContext="customerlandingzone_node1700529677557").getDynamicFrame()
    ...
    val accelerometertrustedzone_node1700529751867 = glueContext.getSinkWithFormat(connectionType="s3", options=JsonOptions("""{"path": "s3://sparkify-bucket-gin/accelerometer/trusted/", "partitionKeys": [], "enableUpdateCatalog": true, "updateBehavior": "UPDATE_IN_DATABASE"}"""), transformationContext="accelerometertrustedzone_node1700529751867", format="json")
    accelerometertrustedzone_node1700529751867.setCatalogInfo(catalogDatabase="sparkify", catalogTableName="accelerometer_trusted")
